...
* Mandatorily Pass an extra hashref to the txn_do method to indicate a UserId, Description and ShardId for the transaction:
...
The L<PgLogLogSet|DBIx::Class::Schema::PgLog::Structure::LogSet> is an identifier for a transaction which has optional Description, mandatory UserId and ShardId. 
	...
	Indexes:
	    "LogSet_pkey" PRIMARY KEY, btree ("ShardId", "Id")
...
The L<PgLogLog|DBIx::Class::Schema::PgLog::Structure::Log> table is main table which stores the Table, TableAction, Columns and its Old and New Values for each transaction. 
	...
	--------------+--------------------------+----------------------------------------------------
	 ...
	 TableShardId | smallint                 | not null
	 ...
	 UserId       | integer                  | not null
	Indexes:
	    "Log_pkey" PRIMARY KEY, btree ("ShardId", "Id")
